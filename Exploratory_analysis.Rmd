...
# vis_all   Графики индивидуальной динамики показателей   (индекс атарогенности, , ИМТ,   давление, пульс,  ) ДО ИМПУТАЦИИ
...
data_spagetty_before_imp %>% filter(!is.na(bmi)) %>% ggplot() +
  geom_line(aes(x = id_point, y = bmi, group = id),
            col = "grey50",
            alpha = 0.4) +
  theme_bw() +
  geom_boxplot(aes(x = id_point, y = bmi),
               fill = "#DEB0D0",
               alpha = 0.8, 
               outlier.size = 0.5)

Динамика ИМП по визитам


# mputed_data_longest_mean Графики индивидуальной динамики показателей  Спагетти (светлосерый, а потом  боксплоты)  (индекс атарогенности, , ИМТ,   давление, пульс,  ) ПОСЛЕ ИМПУТАЦИИ
