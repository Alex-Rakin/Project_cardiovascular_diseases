...
```{r}
ggplot(results, aes(x = reorder(variable1, p.value), y = p.value, color = variable2)) +
  geom_point(size = 3) +  # Точки для p-значений
  geom_hline(yintercept = 0.05, linetype = "dashed", color = "red") +  # Линия уровня значимости (0.05)
  theme_minimal() +
  labs(
    title = "Диаграмма точек для p-значений",
    x = "Переменная 1",
    y = "p-значение",
    color = "Переменная 2"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_log10()  # Логарифмическая шкала, чтобы лучше увидеть различие в значениях
...
# Оставляем только значимые результаты (p-value < 0.05)
edges <- results %>%
  filter(!is.na(p.value) & p.value < 0.05) %>%
  select(variable1, variable2)

# Создаем граф из значимых соединений
...
plot(
  ...
  edge.arrow.size = 0.5,
  main = "Сетевой график значимых результатов (p-value < 0.05)"
)
...
# Преобразуйте результаты в формат «длинной таблицы»
heatmap_data <- results %>%
  filter(!is.na(p.value)) %>%  # Исключение невалидных тестов
...
ggplot(heatmap_data, aes(x = variable1, y = variable2, fill = p.value)) +
  geom_tile(color = "grey") +  # Квадраты тепловой карты
